        ...
        try {
          ...
          if (response.status === 'success' && response.data) {
            if (response.data.signatureUrl) {
              setSignatureFromServer(response.data.signatureUrl);
              // Also update localStorage for consistency
              localStorage.setItem('userSignature', response.data.signatureUrl);
            }
          }
        } catch (error) {
    ...
    switch (currentStep) {
      ...
      case 8:
        ...
        if (formData.idVerification.signature && !localStorage.getItem('userSignature')) {
          localStorage.setItem('userSignature', formData.idVerification.signature);
        }
